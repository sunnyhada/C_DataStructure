...
/*자료구조 2. 양방향 연결 리스트 (Doubly linked list)*/
...
LIST *destroyList(LIST *pList)
{
	...
	NODE *freeNode = pList -> head -> Rlink;
	...
	else
	{
		while(i < pList -> cnt)
		{
			...
			if(pNode -> data > nNode -> data)
			{
				...
				nNode -> Llink = pNode -> Llink;
				pNode -> Llink -> Rlink = nNode;
				nNode -> Rlink = pNode;
				pNode -> Llink = nNode;
				pList->cnt++;
				return 0;
	...
	while ( i < pList -> cnt )
	{
		if(pNode -> data == Key)
		{
			...
			*dataOut = pNode -> data;
			if(pNode -> Rlink == NULL)
			{
				pNode -> Llink -> Rlink = NULL;
				free(pNode);
				pList -> cnt --;
				return 1;
			}
			pNode -> Llink -> Rlink = pNode -> Rlink;
			free(pNode);
			pList -> cnt --;
			return 1;
		}
